import './ThirdPage.css'
const Third=()=>{
return(
    <>
    <div className='box3'>
        <p className='adv'>ADVANTAGES</p>
        <h1>Why Choose Uifry?</h1>
        <p className='Heading2'><img className="icon4" src="/public/images/bell.png"/><img className="icon3" src="/public/images/Ellipse 2158.png"/>Clever Notifications</p>
        <p className="txt">Arcu at dictum sapien, mollis. Vulputate sit id accumsan,<br/> ultricies. In ultrices malesuada elit mauris etiam odio. Duis<br/> tristique lacus, et blandit viverra nisl velit. Sed mattis<br/> rhoncus, diam suspendisse sit nunc, gravida eu. Lectus eget<br/> eget ac dolor neque lorem sapien, suspendisse aliquam.</p>
    </div>
    <img className="star5"src="/public/images/star 1.png"/>
    <div className='box4'>
      <img className="phone5"src="/public/images/phone5.png"/>
      <img className="rings3"src="/public/images/rings3.png"/>
      <img className="el5"src="/public/images/el1.png"/>
   
    </div>
    
    </>
)
}
export default Third;